function [strain_vec,sigma_vec,TIME]= PlasticityMainJ2(matprop,STRAIN,SIGMA,TimeTotal,istep)
...
q_vec=zeros(size(STRAIN));
qbar_vec=zeros(size(STRAIN));
...
for i=1:size(STRAIN)-1
    ...
    if visc==0
        [sigma_vec(i),int_vars_nn1]=maps_plasJ2(matprop,sigma_vec(i-1),eps_rate(i),int_vars_nn1,delta_t);
    else
